...
def refine_pin_detection_adjusted(img):
    ...
    if len(green_pin_centers) != 0:
        if distance >= 95 and distance <= 112 and distance2 < 100:
            ...
            time.sleep(2)
            GPIO.output(13,True)	  
        elif distance > 112 and distance <= 131:
            ...
            classify_found = "6"
            comfirm = 1
        elif distance > 95 and distance <= 112 and distance2 > 100 and distance2 < 200:
...
def main():
	ser2 = serial.Serial(port = '/dev/ttyS0',
				baudrate = 115200,
	...
	while True :
		...
		#adjusted_contour_frame, adjusted_num_pins, shape, classify_found = refine_pin_detection_adjusted(img)		#key = cv2.waitKey(1)
		#if key == ord('p'):
			#if img is not None :

				#adjusted_contour_frame, adjusted_num_pins, shape, classify_found = refine_pin_detection_adjusted(img)
				#print(classify_found)
		if ser2.inWaiting() > 0 :
			command = ser2.readline()
			print(command)
			if command == b"image\n" :
                                time.sleep(1)
                                image = load_image(cap)
                                x = True if image is not None else False
                                while image is None :
                                    image = load_image(cap)
                                #cv2.imwrite('test.jpg',image)
                                adjusted_contour_frame, adjusted_num_pins, shape, classify_found = refine_pin_detection_adjusted(image)
                                print(classify_found,x)
                                ser2.write(classify_found.encode())
			elif command == b"check\n" :
				ser2.write("exist... pin".encode())
